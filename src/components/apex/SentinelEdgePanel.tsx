      ...
      <div className="mt-4 rounded-lg border border-border/50 bg-background/30 p-3">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg border border-border/60 bg-background/40 p-2.5">
            ...
            >
              {item.regimeReport
                ? `${item.regimeReport.state} (${item.regimeReport.currentRegime.regimeId})`
                : "UNAVAILABLE"}
            </p>
            <p className="mt-0.5 text-[10px] text-muted-foreground">
              {item.regimeReport
                ? `Stability ${item.regimeReport.stabilityScore}/100 · ${item.regimeReport.shouldDiscountOldEvidence ? "Discounting stale ticks" : "Full tick history active"}`
                : "No changepoint read"}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border/60 bg-background/40 p-2.5">
            ...
            <p className="mt-1 font-display text-sm font-bold" style={{ color: "var(--bull)" }}>
              {item.calibration
                ? `${(item.calibration.calibratedWinProbability * 100).toFixed(1)}% win`
                : "UNAVAILABLE"}
            </p>
            <p className="mt-0.5 text-[10px] text-muted-foreground">
              {item.calibration
                ? `${item.calibration.method} (N=${item.calibration.sampleSize}) · ${(item.calibration.expectedEdgePp >= 0 ? "+" : "") + item.calibration.expectedEdgePp.toFixed(1)}pp edge`
                : "No calibration read"}
            </p>
          </div>
