...
export function rankOpportunities(
    ...
    for (const c of intel.contracts) {
      ...
      const losingStrengtheningDigits = losers.filter(
        (d) =>
          ...
          (pressureField.velocity && pressureField.velocity[d] > 0.018) ||
          (pressureField.impulse && pressureField.impulse[d] > 0.025),
      );
      ...
      const fusionInputs: EngineEvidenceInput[] = [
        {
          ...
          confidence:
            digitPsychology.verdict === "PERFECT ALIGNMENT"
              ? 95
      ...
      const calibration = calibrateScore(rawScoreClamped, pastTrades, {
        symbol: intel.symbol,
        contract: c.label,
        regime: regimeReport.currentRegime.regimeId,
  ...
  for (const r of ranked) {
    ...
    r.calibration = calibrateScore(r.score, pastTradesFinal, {
      symbol: r.symbol,
      contract: r.contract.label,
      regime: r.regimeReport?.currentRegime.regimeId,
