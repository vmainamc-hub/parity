...
import { sweepThresholds, type SweepReport } from "../precision-digit/threshold-sweep";
import {
...
function evaluateHypothesis(
  ...
  if (decorReport.confidencePenalty > 0) {
    reasoning.push(
      `Decorrelation: -${decorReport.confidencePenalty.toFixed(1)}% collinearity penalty (${decorReport.effectiveVotes.toFixed(1)} effective votes / ${decorReport.rawEvidenceCount} raw).`,
    );
  }
  ...
  for (const s of pred.supports) {
    ...
    if (market) {
      recordForwardEvaluation({
        market,
        engine: s.engine,
        predictedSide: s.contract === "BUY_EVEN" ? "EVEN" : "ODD",
        observedSide: realizedParity,
  ...
  for (const c of pred.conflicts) {
    ...
    if (market) {
      recordForwardEvaluation({
        market,
        engine: c.engine,
        predictedSide: c.contract === "BUY_EVEN" ? "EVEN" : "ODD",
        observedSide: realizedParity,
...
export function analyseMarketParity(
  ...
  if (!enough) {
    ...
    const hmm = fitParityHMM(digits, market);

    // 3. SMC Particle Filter (Latent bias tracking & effective sample size)
    const particles = runParticleFilter(digits, chosenContract);

    // 4. Structural Break & Drift Engine (Two-sided CUSUM & Page-Hinkley)
    const drift = runDriftDetection(digits, market, chosenContract);

    // 5. Bootstrap Null-Hypothesis Significance Engine (Block bootstrap & FDR)
    const significance = computeSignificance(digits, chosenContract, 0.95);
    ...
    const conformal = computeConformalInterval(calibration.calibratedProbability, market);
    ...
    const digitPlan = arbitrateDigitEntry({
      market,
      simReport,
      tensorReport,
      hazardReport,
    ...
    const validation = getValidationDashboardPayload(market);
    ...
    if (forecast.streakProtection.block) {
      ...
      reasons.push(
        `Particle Filter REJECTS — Particle weight collapse (ESS ${particles.essPercent.toFixed(1)}% < 10%).`,
      );
      state = "REJECTED";
      ...
      reasons.push(
        `Conformal 90% Bound: [${(conformal.intervalLow * 100).toFixed(1)}%, ${(conformal.intervalHigh * 100).toFixed(1)}%] (width ${(conformal.intervalWidth * 100).toFixed(1)}%).`,
      );
      reasons.push(
